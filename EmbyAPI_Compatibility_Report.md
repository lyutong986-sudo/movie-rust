...
## 三十二、第三十二轮：扫描状态刷新页恢复 + Episode 详情页对齐 jellyfin（2026-04-24）

> 问题 1：刷新页面后"媒体库扫描任务"面板状态丢失（`scanOperation` 只存在内存 `ref`）。
> 问题 2：Episode 详情页缺少"回 Series"链接、没有集号标签、无上下集导航。

### A. 扫描状态恢复（`frontend/src/store/app.ts` + `settings/LibrarySettings.vue`）

新增 `hydrateScanOperation()`：

1. `isAdmin` 时调用 `GET /api/admin/scan/operations?Limit=10`（后端已实现）
2. 优先挑未完成的任务（`Queued || Running || !Done`），否则取最近一条填入 `scanOperation`
3. 若未完成，直接 `startScanPolling(id)`，UI 显示实时进度

接入点：

- `enterHome()` 结束时 `void hydrateScanOperation()`（登录后首次进入首页触发）
- `LibrarySettings.vue::onMounted` 与其他管理数据一起并发 hydrate

### B. Episode 详情页 UI 改造（参考 `jellyfin前端模板/packages/frontend/src/pages/item/[itemId].vue`）

Jellyfin 的做法：Episode 进入 `/item/[id]`，顶部 `currentSeries.Name` 做成 `RouterLink` 跳 `/series/[id]`。我们对齐：

| 改造 | 说明 |
| --- | --- |
| 标题上方"Series 名"可点击回剧集 | `item.Type === 'Episode' && item.SeriesId` 时用按钮 + `i-lucide-arrow-left` + SeriesName 链接到 `/series/{SeriesId}`。 |
| 标题加 **S01E05** 集号标签 | 根据 `ParentIndexNumber` + `IndexNumber` 计算，PadStart 2 位。 |
| 新增"上一集 / 下一集 / 查看剧集详情"按钮 | 从已有的 `relatedItems`（同季）按 `IndexNumber` 排序定位当前条目前后集；直接 `openChild` 切换到那条 Episode。 |
| `AppLayout.vue` 面包屑 | Episode 时的 `SeriesName` 段从纯文本改为 `{ label, to: /series/{SeriesId} }`，和顶部其它面包屑统一渲染为可点击。 |

### C. 结果

- 扫描进行中刷新页面，面板回显 Queued/Running + Progress；全完成的任务也会显示最近一次的状态（不会回到 Idle 0%）。
- 进入 Episode 详情页：顶部可点击"← 剧集名"回 Series；标题带 `S01E05`；下面一排 上一集 / 下一集 / 查看剧集详情 按钮；顶栏面包屑 `首页 › 剧集名 › 分集名`，剧集名可点。
...
## 三十三、第三十三轮：冗余代码/垃圾代码清理（2026-04-24）

> 目标：系统审计两端 dead code + warnings，删除不再被任何地方引用的函数/结构体、重写死变量、清理无用依赖与 build chunk。

### A. 后端清理清单（14 个文件，净 **-720 行**）

| 位置 | 删除/重构内容 |
| --- | --- |
| `media_analyzer.rs` | 删除未用的 `extract_primary_video_stream`、`extract_audio_streams`、`extract_subtitle_streams`；删除枚举死 variant `NoVideoStream` / `NoAudioStream`。 |
| `metadata/person_service.rs` | 删除未用的 `PersonService::fetch_persons_for_item`。 |
| `metadata/provider.rs` | 删除未用的 `MetadataProviderManager::providers`。 |
| `metadata/tmdb.rs` | 删除未用的关联函数 `TmdbProvider::new`。 |
| `models.rs` | 删除无引用的 `DbSeriesEpisodeCatalog`、`DbPersonRole`、`ApiKeyInfoDto`、`NameIdDto`、`SeasonDto`、`EpisodeDto`；删除 `optional_uuid_to_emby_guid`、`try_emby_id_to_uuid`。 |
| `repository.rs` | 删除无调用点的 `list_all_sessions`、`create_api_key_session`、`delete_api_key_session`、`get_user_resume_items`、`media_streams_for_item`、`video_display_title`、`name_id_items_from_names`；修复 `get_persons(...)` 里 `let mut query` 的"赋值后立即被覆写"死写，改成单次 `let = if ... else`；删除 `total_bitrate` 无意义累加变量。 |
| `routes/images.rs` | 删除未挂到 router 的旧 handler：`get_item_image_index_url`、`delete_item_image_index`、`set_item_image_index`、`get_item_image_extended`、`delete_user_image_index` 及三个配套 Path 结构体。 |
| `routes/metadata_routes.rs` | 清理 `Path`、`StatusCode`、`MetadataProviderManager` 未用 import。 |
| `routes/websocket.rs` | 删除未用的 `pub type Sessions` 别名以及 `HashMap / Arc / RwLock` 未用 import。 |
| `transcoder.rs` | 删除未用的 `Transcoder::cleanup_expired_sessions`；把三处 `if let Some(mut session) = sessions.get_mut(...)` 的多余 `mut` 去掉；清理 `Duration` 未用 import。 |

编译 warnings：65 → **27**（剩余全部是对外 DTO 字段 / 向未来扩展预留的合约，属于"保留"类）。

### B. 前端清理

| 位置 | 变更 |
| --- | --- |
| `frontend/package.json` | `npm uninstall zod`，代码里完全没 import。 |
| `frontend/vite.config.ts` | 删除 `manualChunks.zod`，不再产生 0-byte 空 chunk `zod-l0sNRNKZ.js`。 |
| `frontend/dist/`（Git 跟踪过的旧构建产物） | 已在 `.gitignore` 忽略；无需再 commit。 |
| `.gitignore` | 新增 `auto-imports.d.ts`、`components.d.ts`、`.nuxt-ui/`、`.scripts/` 等自动生成 / 临时脚本目录。 |

### C. 校验

- `backend> cargo build` —— ✅ 通过（27 warnings，全为合约字段）
- `frontend> npm run build` —— ✅ 通过（不再出现 "Generated an empty chunk: zod"）
- 文件 lint —— ✅ 无错误
- `git diff --stat`：14 个文件、**-720 行 / +46 行** 的净精简。

## 三十四、第三十四轮：继续审计清理（废弃工具 / 过时文档 / 未用 API，2026-04-24）

> 在第三十三轮的基础上继续审计：探测废弃的一次性工具、过时的设计文档、还没被任何页面调用的 API 入口。

### A. 废弃一次性工具

| 位置 | 处理 |
| --- | --- |
| `backend/src/bin/check_tables.rs` | 一次性 DB 探测脚本，硬编码检查 4 张表是否存在。`ensure_schema_compatibility` 已经在启动时自检同样内容，此文件是早期调试遗留物。**删除**；同时移除空的 `backend/src/bin/` 目录。 |

### B. 过时设计文档

| 文档 | 状态 | 处理 |
| --- | --- | --- |
| `docs/API_COMPAT.md` | 内容停留在"当前播放接口是 Direct Play / Direct Stream，暂未实现转码"等早期阶段，与现在的 HLS 转码 / STRM 支持 / Playback Operation 模型矛盾。 | **删除** |
| `docs/JELLYFIN_FRONTEND_PARITY.md` | 内容停留在"API Key 仍是兼容占位页"、"人物元数据还没持久化"等，与 P0/P1/P2 全量落地后的现状不符。 | **删除** |
| 空的 `docs/` 目录 | 无内容 | **删除** |
| `README.md` | 引用了上述两个已删文档 | 改为指向根目录统一的 `EmbyAPI_Compatibility_Report.md`。 |

### C. 前端未用 API / store 导出

| 位置 | 清理 |
| --- | --- |
| `frontend/src/api/emby.ts::updateUserData(...)` | 无任何调用点（favorite / played 用独立的 `markFavorite`、`markPlayed`）。**删除**。 |
| `frontend/src/store/app.ts::currentItems` computed | 无外部使用，仅自身定义。**删除**。 |
| `frontend/src/store/app.ts::libraryCards` computed | 无外部使用，`libraries.value` 已能直接用。**删除**。 |

### D. 运行期产物进入 gitignore

- `.gitignore` 新增 `log/` 与 `logs/` 两个运行期目录（之前只有 `*.log` 匹配文件，目录仍可能被误提交）。

### E. 仍保留（有引用或合约所需）

- `serializeParentStack` / `hydrateParentStack`：`library/LibraryPage.vue` 用来把面包屑栈编码进 URL 支持刷新还原。
- `libraryGenresCache`：`loadLibraryGenres` 的 memoization，被 LibraryPage 调用。
- 后端 DTO 字段未读的 warnings：全部是 Emby 客户端 / 外部 API 的合约字段（SerasonsQuery / EpisodesQuery / RemoteImagesQuery 等），保留以正确反序列化。

### F. 校验

- `backend> cargo build` —— ✅ 通过（27 warnings，全为合约字段）
- `frontend> npm run build` —— ✅ 通过
- `git diff --stat`：加上本轮，两轮合计 **21 个文件、-1027 行 / +89 行**。

## 三十五、第三十五轮：主内容区链路审计（AppLayout `<slot/>` 容器相关页面，2026-04-24）

> 审计范围：`AppLayout.vue` 的主容器 `flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto p-4 sm:p-6` 中所有经过 `<RouterView>` 渲染的页面数据加载链路。

### A. 发现并修复的问题

#### A1. `LibraryPage` 切库**重复 `loadItems()`**（P0）

原链路：

```ts
// watch(route.params.id, immediate)
await selectLibrary(value);   // 内部已经 loadItems()
await hydrateParentStack(...);
await loadItems();            // 重复
await loadLibraryGenres(...);
```

修复：把 `selectLibrary()` 的状态更新内联到 watcher 里（只改 `state.selectedLibraryId` / `state.search` / `parentStack`），后面统一 `loadItems()` 一次；`store::selectLibrary` 没有其它调用方，已删除。

#### A2. `AppLayout` 对 admin 子页每次跳转都 `loadAdminData()`（P0）
...
watch(() => route.fullPath, async () => {
  ...
  await loadAdminData();  // 每次跳转都拉 system/users/cultures/startup
});
```

修复：改为监听 `isAdminSection` 的 **边沿**（从 false 变 true 才加载）。进入 admin 区后在 `/settings/a → /settings/b` 之间来回切页不再重复拉 4 份数据。

#### A3. `SeriesPage` 串行 N+1（P1）

原链路 `seasons → episodes(N 并发，但等完成) → nextUp → related → similar` 完全串行，5 季 + 相关 + 相似要跑 `1 + 5 + 1 + 1 + 1 = 9 轮` 串行。

修复：`Promise.all([seasonEpisodes, nextUp, related, similar])`，四个独立请求整体并行。N 季剧从 9 轮减到 **2 轮**（季列表 → 并行四组）。

#### A4. `ItemPage` 串行（P1）

原链路 `item → children → related → similar` 四轮串行。

修复：`item` 后 `Promise.all([childPromise, relatedPromise, similarPromise])`，失败 silent 回落空数组。

#### A5. `loadLatestByLibrary` tvshows 排序名不符实（P2）

原：tvshows 用 `SortName Asc` 当 "最新剧集"。

修复：改为 `DateCreated Desc`，和 "最新剧集" 语义一致（与 `loadRecentlyAddedTitles` 的 tvshows 一致）。

#### A6. `loadItems` 无谓字段 `Overview`（P2）

列表卡片不显示简介，`fields` 去掉 `Overview`，减小 `/Users/{id}/Items` 响应体积。保留 `MediaStreams`（给质量徽章）、`MediaSources`、`ChildCount`。

#### A7. 废代码 `store::openItem`

所有页面都用本地 `router.push(itemRoute(item))`，store 的 `openItem` 依赖已删除的 `selectLibrary`，整体没人调用——**删除**。

### B. 未修但记录

| 项 | 说明 |
| --- | --- |
| `loadItems` 无 AbortController | 用户快速切换筛选/库时旧请求不会取消，后发先到的竞态会导致列表闪。留给后续用请求版本号解决。 |
| `TopLoader` 只 hook `state.busy` | ItemPage / SeriesPage / PlaylistsPage 等用本地 `loading` 的页面不会触发全局顶部进度条。统一需要把这些页面改走 `store::run`，UX 一致性收益中等。 |
| `loadRecentlyAddedTitles` + `loadLatestByLibrary` 对 movies 库发两次相同请求 | 两者内容高度重叠（movies 部分完全一样）。可合并为"先 latestByLibrary，再 flatten + slice(36)"得到 recentlyAdded。N 个库场景下可省 N 个请求。 |

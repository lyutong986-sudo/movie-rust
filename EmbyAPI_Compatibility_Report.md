...
> ④ **Sakura `/myinfo` 与播放榜统计**：`POST /emby/user_usage_stats/submit_custom_query` 中单用户/排行模式改为按「播放实例」（`play_session_id` 退化到 `session_id`）+ `item_id` 聚合；**有 `position_ticks` 进度时用 tick 差折算分钟/秒**（贴近 Emby PlaybackActivity），否则回退到首尾时间差；Sakura SQL 内日期按 **UTC+8** 解析以匹配 `emby_cust_commit`。**说明**：面板「在线人数」在 Sakura 侧 `get_current_playing_count` 带 **120s** 缓存（`@cache.memoize(ttl=120)`），非实时。

...
> 对比时间：2026-05-03（⑧ **PB50 补审 — Stream URL `/emby/` 前缀全量审计**：对接 ⑦ 修复后再做项目级审计，专门扫所有"会被回写到客户端、客户端会用 `Uri.resolve` 拼绝对地址"的相对 URL，又发现并修复 3 处遗漏：(a) `routes/videos.rs::rewrite_hls_playlist` —— HLS 转码 segment URL 在 playlist 内重写时只写 `/{Videos|Audio}/{id}/hls1/{sid}/{seg}`，反代下客户端拉 segment 全 404；改写为 `/emby/{Videos|Audio}/...`；(b) `routes/videos.rs::hls_audio_playlist_response` 直转 audio playlist 内联 `/Audio/{id}/hls1/main/0.ts` 缺前缀；(c) `routes/images.rs::item_image_url_response` `/Items/{id}/Images/{type}/Url` 端点回的 `Url` 字段一直是 `/Items/.../Images/...`，反代下客户端拿到这个 URL 也会丢路径；统一改成 `/emby/Items/...`。其余路径（`build_direct_stream_url`、`build_transcoding_url`、`media_source_for_item`、`get_media_source_with_streams` 的 `Path` / `DirectStreamUrl` / 字幕 `DeliveryUrl`、`/Videos/{id}/main.m3u8` 内联 segment、`subtitles_playlist` 等）在 ⑦ 已修复。**外部 URL（TMDB、ExternalUrls、RemoteImage 候选、`/api/admin/...`）保持原样不加前缀，符合 Emby 标准。** **响应字段审计**：对照 EmbySDK `MediaSourceInfo` 字段一一比对 backend `MediaSourceDto` —— 所有关键字段（`SupportsDirectPlay/Stream/Transcoding`、`Container`、`Bitrate`、`Size`、`RunTimeTicks`、`MediaStreams`、`Chapters`、`RequiredHttpHeaders` 等）均已正确填充；`PlaybackInfoResponse` 的 `MediaSources` / `PlaySessionId` / `ErrorCode` / `TranscodingInfo` 全部齐全。涉及 `backend/src/routes/videos.rs`、`backend/src/routes/images.rs`。⑦ **PB50 — Stream URL `/emby/` 前缀缺失（Filmly "获取播放地址失败"修复）**：对照 EmbySDK `MediaSourceInfo` 与 Emby 4.9.3 服务器实测响应，所有 stream / subtitle URL **必须带 `/emby/` 前缀**（与官方一致）。我们历史上把 `Path` / `DirectStreamUrl` / `Subtitles/.../Stream.x` 写成了 `/Videos/...`、`/Videos/.../Subtitles/...`，客户端用 `Uri.parse(baseUrl).resolve(...)` 拼播放地址时绝对路径会**整段覆盖** `base.path`——若 baseUrl 是 `https://host/sub/emby/` 这种反代/子路径部署，结果丢失 `/sub/emby` 段，请求落到 404。Filmly 表现是「获取播放地址失败」。修复：`build_direct_stream_url`、`build_transcoding_url`（已是）、`media_source_for_item` 的 `sanitized_path` 与 `direct_stream_url`、`get_media_source_with_streams` 同字段、`subtitle_descriptor` 的 `DeliveryUrl`、HLS playlist 内联 `main.m3u8` / `hls1/main/0.ts` URL、`subtitles_playlist` 的 `Subtitles/.../Stream.{ext}` URL 全部加 `/emby/` 前缀；同时更新 `playback_info_builds_emby_original_direct_stream_urls_for_local_player` 单测断言到新前缀。涉及 `backend/src/routes/items.rs`、`backend/src/routes/videos.rs`、`backend/src/repository.rs`。⑥ **`GET /Shows/{id}/Seasons`**：`media_item_to_dto_for_list` 此前对季使用 `DtoCountPrefetch::default()`，Folder 型 Season 的 **`ChildCount` / `RecursiveItemCount` 恒为 0**，部分仅读顶层计数、不读 `ProviderIds.RemoteChildCount` 的 Emby 系客户端会误判「无分集」进不了详情。现对当前页全部 Season `id` 调用 **`repository::count_item_children_batch`** 一次 SQL 批量回填两计数；并对季列表应用 **`apply_season_response_shape`**，使 `EnableImages` / `EnableUserData` / **`Fields`** 与 Episodes 列表裁剪逻辑一致。涉及 `backend/src/routes/shows.rs::get_seasons`；① 远端 Emby 流量模式 redirect / redirect_direct 严重 bug 修复 — `proxy_item_stream_internal_with_source` 历史误把 `ensure_authenticated()` 返回的 `remote_user_id` 当成 token 拼进 `api_key=`，改为从 `source.access_token` 取真实 token，并补强 redirect_direct 失败重试 + 降级回 redirect 的双重兜底；② 前端集成 mpegts.js — 浏览器播放 FLV / MPEG-TS 完全走客户端 demux + MSE，不再依赖服务端转码，按容器/编码智能选播放引擎）

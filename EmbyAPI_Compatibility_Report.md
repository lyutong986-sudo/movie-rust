...
> ⑤ **2026-05-03 日志审计修复（`log/movie-rust-20260503214155.log`）**：
> - **图片接口接受上游原始 itemId 兜底**：`/Items/{id}/Images/...` 系列在 `emby_id_to_uuid` 解析失败时，新增 `repository::find_item_id_by_remote_emby_id` 反查 `provider_ids->>'RemoteEmbyItemId'` / `'RemoteEmbySeriesId'`。Hills 等客户端从上游 Emby 切到本后端时 UI 缓存里残留的数字 itemId（76 次 client 400 / 33 个不同 ID）从此命中本地 UUID 走通常代理路径，不再 `400 无效的项目ID格式: <num>`。涉及函数：`serve_item_image / list_item_images / item_image_url_response / get_item_thumbnail_set`。
> - **`/Sessions/Playing*` 上报兼容多 Content-Type**：新增 `LenientPlaybackReport` extractor，按 JSON → form-urlencoded → query string → 默认 顺序兜底；`PlaybackReport` 加 `#[derive(Default)]`。修复 Hills 客户端 `Content-Type` 缺失/异常时被 axum `Json` 拒成 415 导致播放上报丢失（日志 1 次 `status=415`）。涉及 4 个 handler：`playback_started / playback_ping / playback_progress / playback_stopped`。
> - **图片 404 错误链不再嵌套上游 5xx**：`AppError::NotFound` 不再把 reqwest 的 `500 Internal Server Error` 字符串拼进消息，避免日志里出现 `status=404 Not Found error=...: 500 Internal Server Error` 这种自相矛盾的提示。上游状态改记 `tracing::debug!`。涉及 `routes/images.rs`（3 处）+ `http_client.rs`（1 处）。
> - **PB42-bo：sidecar 图片下载源级 backoff + URL 短窗口去抖**：`run_remote_emby_sidecar_download_pass` 引入两层防御：①每个 source 维护 `consecutive_failures` 计数，连续失败 ≥ 8 次进入指数退避（30s → 60s → ... → 600s 上限）暂停窗口；②单 URL 在 10 分钟内连续失败 ≥ 3 次会被本地标记，本轮直接跳过。任意一次成功立即清空两侧状态。日志中 sidecar 502/500 风暴（≈2666 次/14min）从此不再持续打上游也不再刷屏。
> - **`Shows/NextUp` 慢 SQL 重写**：删除 `EXISTS (SELECT 1 FROM media_items season WHERE season.id = mi.parent_id AND season.parent_id = $2)` 兜底子查询。`ensure_schema_compatibility` 启动时已强制回填 Episode.series_id，此 EXISTS 与 `mi.series_id = $2` 等价但会让 Postgres 在 BitmapOr 之外多跑嵌套子查询（实测把单次 NextUp 由 ~10ms 拖到 1.0–1.4s，日志 22 条慢查询里至少 18 条命中此模式）。剩下的 3 路 OR 都直接落 b-tree 索引。
> - **`Items/Resume` COUNT 慢 SQL 修复**：`fast_count_media_items` 新增 `resume_only` 短路。原来 fallback `SELECT COUNT(*) FROM media_items WHERE 1 = 1` 会扫整张表（日志中 1.04s+），新路径走 `user_item_data uid WHERE user_id=$1 AND playback_position_ticks > 0 AND EXISTS (...)`，driving table 是 `user_item_data` 的 PRIMARY KEY range scan，典型 in-progress 行 <100 条 → 实测 <50ms。
> - **PB42-IC：图片代理本地磁盘缓存 + 稳定 ETag**：`serve_remote_image` 新增 `cache_dir/<bucket>/<sha256-key>.bin` + `.ct` 双文件缓存层；`cache_key` 用剥掉 `api_key` / `X-Emby-Token` 的稳定 URL + transform 参数 hash 出来。所有响应都返回基于 `cache_key` 的稳定 ETag（`"img-<key>"`），`If-None-Match` 命中直接 304（不打盘也不打 upstream）。新增 `image_cache_eviction_loop` 后台任务每小时清理过期文件（默认 TTL=7 天，可通过 `APP_IMAGE_CACHE_TTL_SECS` 调整）。Config 增 `image_cache_dir`（`APP_IMAGE_CACHE_DIR`，默认 `image_cache`）+ `image_cache_ttl_secs`。Hills 主页瞬间发 100+ 张图请求场景下，二次访问 / 多客户端共享缓存命中率接近 100%。
> ⑥ **2026-05-04 Hills 等 Emby 第三方客户端翻页失效修复（StartIndex 字段非 SDK 兼容性）**：
> - **现象**：Hills（`X-Emby-Client=Hills`）在请求 `/Users/{id}/Items?Limit=60&StartIndex=0&ParentId=...&IncludeItemTypes=Series&Recursive=true` 时只加载第一页 60 条就停止翻页，而 AfuseKt（`X-Emby-Client=AfuseKt/...`，使用 `Limit=30`）能正常翻到 `StartIndex=420+`。后端日志确认两个客户端相同条件下都返回 `TotalRecordCount: 660` 正确总数。
...
| **PB49 (FX1) 层级数量 fast-skip 不再被 `is_incremental` 闸门限制** | 旧实现里 `series_counts_cache` / `season_counts_cache` 只在增量模式（`last_sync_at IS NOT NULL`）才加载使用；导致「首次同步跑了一半 → 容器/网络中断 → 重试」场景退化为 per-Episode 的 `local_synced_ids` 慢路径，每个 Series 都会重新拉一遍 Seasons + Episodes 详情 HTTP（用户看到「正在处理 变形金刚」卡很久）。修复后无条件预加载缓存，命中即跳过整个 Series/Season 远端拉取链；缓存为空时（真正的首次同步）行为退化到全量逻辑、不影响正确性。同时增加大 Series（`season_count >= 3`）进入处理时的 INFO 心跳日志，避免日志静默 | ✅ 已修复 |
| **PB49 (FX2) view 续抓游标实际接入 sync_source_inner** | `repository::get_view_progress` / `save_view_progress` / `clear_source_view_progress` 之前只有定义没人调用。本次接入：(1) 电影库 paginator 启动时按 `(source, view, incremental_since)` 读取游标续抓，每抓一页全部入库后写回新游标；(2) 电视剧库全量模式 Series-list paginator 同样接入；(3) 增量模式 TVShows 仍走 DateModified 早停（更优）；(4) 整次同步成功完成时 `clear_source_view_progress` 重置所有 view 游标。配合 FX1 后，重试/中断恢复速度由"分钟级"降到"毫秒级" | ✅ 已修复 |
| **PB49 (FX3) 启动期标记被中断的首次同步源** | 容器 SIGKILL / 进程 panic 时 `last_sync_error` 不会被写入，导致下次容器启动后前端按钮仍显示「立即同步」而不是「重试同步」。在 `main.rs` 的 axum::serve 启动前、scheduler/auto-sync/monitor 等 spawn **之前**，扫描 `remote_emby_sources` 中 `last_sync_at IS NULL` 且 `last_sync_error IS NULL` 且 `media_items` 里已有该源条目的行，写入 `last_sync_error="上次同步在进程退出前被中断..."`，让前端 `isSourceFailedLastRun` 正确返回 true、按钮文案切到「重试同步」 | ✅ 已修复 |
| **PB49 (FX4) 前端 pollOperations 优雅处理 404/410** | 旧实现用 `Promise.all` 拉所有 in-flight 操作，容器重启后旧 in-memory `operationId` 全部 404，整批 reject 让 UI 永远卡在「正在同步」。改用 `Promise.allSettled` + 单条异常映射：404/410 直接把对应 SourceId 从 `operationBySourceId` 移除并触发 `refreshSourcesOnly()`（按钮回到 last_sync_at/error 决定的初始态）；其它错误（5xx/网络）保留旧记录、下一轮重试，仅 console 提示一次。同时 `api/emby.ts::request` 抛出的 Error 增加 `status` 字段，避免业务层硬解析 message 字符串 | ✅ 已修复 |
| **PB49 (FX5) 同步任务静默挂起防御 + 实测调参 + 减负载** | 新日志 `movie-rust-20260503225750.log` 暴露真挂死案例：c688de5e 在 14:36:26 完成 series_idx=15 后整个 sync 任务静默挂起 21+ 分钟（无 ERROR / 无 HTTP / 无 SQL / 无 timeout 触发），同样 457a955f 在 14:41:01 完成 series_idx=40 后挂起 16+ 分钟。reqwest 自带的 120s timeout 在某些边界场景（半关连接 / pool 中污染连接）下不触发。在 `remote_emby.rs` 三个关键阻塞点叠加 `tokio::time::timeout` runtime 级硬上限。**v2 实测调参**：基于 `movie-rust-20260503231217.log`（77k 行，2 分钟样本，c688de5e p50=7.7s/p90=18.9s，457a955f p50=6.4s/p90=36s/max=36s，最大已知 Series 蓝猫淘气三千问 7 季 2168 集 ~16s）把 timeout 从 180/180/600 收到 **60/60/120**：(a) `fetch_remote_seasons` 60s（5x P90）；(b) `fetch_remote_episodes_for_season` 60s（覆盖 424 集 Season 5s 实测 + 12x 裕度）；(c) Episode 批量 `buffer_unordered.collect` 120s（24x P50）。任何超时只跳过当前 Series/Season 继续下一条。**v2 减负载**：对齐上游 Emby web 客户端（gyb.emby.yun:4443 抓包验证）的 query 参数：`fetch_remote_seasons` 增加 `EnableUserData=false / EnableTotalRecordCount=false / IsSpecialSeason=false`；`fetch_remote_episodes_for_season` 增加 `EnableUserData=false / EnableTotalRecordCount=false`，免每条 ~150 字节 UserData + 跳过 server 端 COUNT(*) 开销。配套加 `PB49 (FX5-A)` trace log 让"已经走到迭代器第 N 条"事实落到日志，hang 时能锁定到具体 series_idx + name | ✅ 已修复 |
...
## 第二十八轮修复：整数查询参数空字符串反序列化 + 分页兼容

**问题表现：**
- Hills 播放器调用 `GET /Genres?...&Limit&...` 时服务端返回 400："Failed to deserialize query string: Limit: cannot parse integer from empty string"
- 播放器分页浏览（`GET /Users/{id}/Items?StartIndex=60&Limit=60&...`）无法翻完全部页面，因 Genres 端点 400 中断了后续浏览流程

**根因分析：**
- Emby 客户端在 URL 中可能发送空值查询参数（如 `&Limit&` 或 `&Limit=&`），serde 的标准 `Option<i32>` / `Option<i64>` 反序列化器无法将空字符串解析为整数，直接返回 400
- 该问题不仅影响 `/Genres`，理论上影响所有使用 `Limit` / `StartIndex` 等整数查询参数的端点

**修复方案：**

| 编号 | 修复 | 文件 | 说明 |
|------|------|------|------|
| R170 | 宽松整数反序列化器 | `models.rs` | 新增 `deserialize_option_i64_lenient` 和 `deserialize_option_i32_lenient`，接受原生数值、字符串整数、空字符串→None、null→None |
| R171 | 全端点 Limit/StartIndex 加固 | 14 个 query struct | 对所有 `Limit`、`StartIndex`、`ImageTypeLimit`、`CategoryLimit`、`ItemLimit` 字段统一应用宽松反序列化器 |

**受影响的 query struct 和文件：**

| 文件 | Struct | 字段 |
|------|--------|------|
| `models.rs` | `ItemsQuery` | `start_index`, `limit`, `image_type_limit` |
| `models.rs` | `EpisodesQuery` | `start_index`, `limit`, `image_type_limit` |
| `models.rs` | `SeasonsQuery` | `image_type_limit` |
| `models.rs` | `GetSimilarItems` | `start_index`, `limit`, `image_type_limit` |
| `models.rs` | `ActivityLogQuery` | `limit` |
| `routes/genres.rs` | `GetGenresQuery` | `start_index`, `limit` |
| `routes/persons.rs` | `GetPersonsQuery` | `start_index`, `limit` |
| `routes/images.rs` | `RemoteImagesQuery` | `start_index`, `limit` |
| `routes/items.rs` | `SearchHintsQuery` | `start_index`, `limit` |
| `routes/items.rs` | `MovieRecommendationsQuery` | `category_limit`, `item_limit` |
| `routes/sessions.rs` | `PagingQuery` | `start_index`, `limit` |
| `routes/sessions.rs` | `SessionCommandQuery` | `start_index`, `limit` |
| `routes/playlists.rs` | `ListItemsQuery` | `start_index`, `limit` |
| `routes/users.rs` | `UserQuery` | `start_index`, `limit` |
| `routes/system.rs` | `LogQuery` | `start_index`, `limit` |
| `routes/admin.rs` | `ScanOperationsQuery` | `limit` |
| `routes/remote_emby.rs` | `DiagnosticSampleItemsQuery` | `limit` |
| `routes/remote_emby.rs` | `SyncOperationsQuery` | `limit` |

**分页验证：**
- Items 端点 `TotalRecordCount` 由 `fast_count_media_items` 独立计算（不受 OFFSET/LIMIT 影响），返回正确的全部匹配数
- 播放器通过 `TotalRecordCount` 判断何时停止翻页：`if (total != 0 && startIndex >= total) break;`
- SQL 分页通过 `OFFSET $start_index LIMIT $limit` 实现，与 count 查询使用相同的 WHERE 条件

`cargo check` 通过（0 errors）。

---

## 第二十九轮修复：筛选端点官方响应对照 + 响应格式修正 + 作用域过滤

**时间**: 2026-05-04

**问题**: 将项目各筛选聚合端点（`/Tags`, `/OfficialRatings`, `/Genres`, `/Studios`, `/Years`）的响应与官方 Emby 服务器实际响应进行逐一对照，发现以下差异：

### 差异分析

| 端点 | 差异描述 |
|---|---|
| `/Tags` | 项目返回完整 `BaseItemDto`；官方仅返回 `{"Name": "..."}` 简单对象 |
| `/OfficialRatings` | 项目返回原始字符串数组 `["AT-14", ...]`；官方返回 `[{"Name": "AT-14"}, ...]` 对象 |
| `/Studios` | 缺少 `UserData`（PlaybackPositionTicks/PlayCount/IsFavorite/Played）字段 |
| `/Genres` | 缺少 `UserData` 字段 |
| `/Years` | 返回多余字段（Id/Type/ProductionYear/IsFolder）；官方仅返回 `{"Name": "2025"}` |
| **全部端点** | 不支持 `ParentId` + `IncludeItemTypes` 作用域过滤（查询参数被静默忽略） |
| **全部端点** | `ImageTags` / `BackdropImageTags` 在为空时被 `skip_serializing_if` 省略；官方始终返回 `"ImageTags": {}`, `"BackdropImageTags": []` |

### 修复内容

**1. 新增 `FilterAggregateQuery` 共享查询结构体** (`routes/items.rs`)
- 支持 `UserId`, `ParentId`, `IncludeItemTypes`, `Recursive`, `SortBy`, `SortOrder`, `Limit`, `StartIndex`
- 所有整数字段使用宽松反序列化（空字符串 → None）

**2. 新增作用域聚合仓库函数** (`repository.rs`)
- `aggregate_array_values_scoped()` — tags/studios/genres 按 library_id + include_types 过滤
- `aggregate_text_values_scoped()` — official_rating/container 按 library_id + include_types 过滤
- `aggregate_years_scoped()` — years 按 library_id + include_types 过滤
- 无作用域参数时自动回退到原有缓存路径

**3. 响应格式修正**
- `/Tags`: `BaseItemDto` → `[{"Name": "..."}]` + `TotalRecordCount`
- `/OfficialRatings`: 原始字符串 → `[{"Name": "..."}]` + `TotalRecordCount`
- `/Years`: 精简为 `[{"Name": "2025"}]` + `TotalRecordCount`；支持 `SortOrder=Descending`
- `/Studios`: 新增 `UserData` 默认数据（PlaybackPositionTicks=0, PlayCount=0, IsFavorite=false, Played=false）
- `/Genres`: 新增 `UserData` 默认数据
- `/Containers`: 同 OfficialRatings，改为 `[{"Name": "..."}]` 格式

**4. `BaseItemDto` 序列化修正** (`models.rs`)
- `image_tags`: 移除 `skip_serializing_if = "BTreeMap::is_empty"`，始终序列化
- `backdrop_image_tags`: 移除 `skip_serializing_if = "Vec::is_empty"`，始终序列化
- 与官方 Emby 保持一致：空时返回 `"ImageTags": {}` 和 `"BackdropImageTags": []`

**5. `UserItemDataDto` 增加 `Default` derive** (`models.rs`)
- 方便在虚拟条目中快速初始化默认用户数据
...
| 文件 | 修改内容 |
|---|---|
| `backend/src/routes/items.rs` | 新增 `FilterAggregateQuery`、`resolve_filter_library_id()`、`name_only_list_result()`、`virtual_folder_item_with_user_data()`；重写 tags/official_ratings/studios/years/containers 处理器 |
| `backend/src/routes/genres.rs` | `genre_to_base_item()` 添加 `UserData` 默认值 |
| `backend/src/repository.rs` | 新增 `aggregate_array_values_scoped()`、`aggregate_text_values_scoped()`、`aggregate_years_scoped()` |
| `backend/src/models.rs` | `UserItemDataDto` 增加 `Default` derive；`image_tags` / `backdrop_image_tags` 移除 skip_serializing_if |

### 官方响应对照结果

| 端点 | 修复前 | 修复后 | 与官方一致 |
|---|---|---|---|
| `/Tags` | 完整 BaseItemDto + 无作用域 | `{"Name":"..."}` + ParentId/IncludeItemTypes 过滤 | ✅ |
| `/OfficialRatings` | 原始字符串 + 无作用域 | `{"Name":"..."}` + 过滤 | ✅ |
| `/Genres` | 缺 UserData/BackdropImageTags | 完整匹配官方格式 | ✅ |
| `/Studios` | 缺 UserData/BackdropImageTags + 无作用域 | 完整匹配 + 过滤 | ✅ |
| `/Years` | 多余字段 + 无作用域 | `{"Name":"2025"}` + 过滤 + SortOrder | ✅ |
| `/Users/{id}/Items` | 已正确 | 不变 | ✅ |

`cargo build` 通过（0 errors）。

---

## 第三十轮修复：UserData.UnplayedItemCount + RecursiveItemCount + AirDays 集数显示

**时间**: 2026-05-04

**问题**: 播放器显示 Series 集数需要 `UserData.UnplayedItemCount`、`RecursiveItemCount`、`Status`、`AirDays` 字段。对照官方响应发现：

### 差异分析

| 字段 | 问题 |
|---|---|
| `UserData.UnplayedItemCount` | **始终为 `null`**，从未计算。播放器无法显示"还有 N 集未看" |
| `RecursiveItemCount` | 列表接口使用 `DtoCountPrefetch::default()` 传 0，已在上轮修复（`media_items_to_dto_result` 中批量预取） |
| `AirDays` | 字段存在但有 `skip_serializing_if = "Vec::is_empty"`，官方始终返回 `"AirDays": []` |
| `CanDownload` | Series/Season 等文件夹类型应为 `false`，项目对所有条目都设为 `true` |

### 修复内容

**1. 新增 `count_unplayed_children_batch()` 仓库函数** (`repository.rs`)
- 使用递归 CTE 查找所有后代，统计用户未播放（`user_item_data.is_played != true`）的子条目数
- 一条 SQL 批量处理所有 folder 类型条目

**2. 列表接口注入 UnplayedItemCount** (`routes/items.rs`)
- `media_items_to_dto_result()` 中的 `tokio::join!` 新增 `count_unplayed_children_batch` 调用
- 在 DTO 构建循环中，对 folder 类型条目注入 `dto.user_data.unplayed_item_count`

**3. 详情接口注入 UnplayedItemCount** (`repository.rs`)
- `media_item_to_dto_inner()` 的 `tokio::try_join!` 新增 `unplayed_count_fut`
- 在 user_data 构建后注入 `unplayed_item_count`

**4. `AirDays` 始终序列化** (`models.rs`)
- 移除 `skip_serializing_if = "Vec::is_empty"`，与官方一致始终返回 `"AirDays": []`

**5. `CanDownload` 修正** (`repository.rs`)
- 列表 DTO：`can_download: !is_folder`（文件夹类型 → false）
- 详情 DTO：同上
...
| 文件 | 修改内容 |
|---|---|
| `backend/src/repository.rs` | 新增 `count_unplayed_children_batch()`；详情 DTO 添加 unplayed_count 计算和注入；`can_download: !is_folder` |
| `backend/src/routes/items.rs` | 列表 DTO 构建添加 unplayed_counts 批量预取和注入 |
| `backend/src/models.rs` | `air_days` 移除 `skip_serializing_if` |
...
{
  "UserData": {
    "UnplayedItemCount": 120,
    "PlaybackPositionTicks": 0,
    "PlayCount": 0,
    "IsFavorite": false,
    "Played": false
  },
  "RecursiveItemCount": 120,
  "Status": "Continuing",
  "AirDays": [],
  "CanDownload": false
}
...
## 第三十一轮修复：图片 404 自动触发 TMDB 元数据/图片补全兜底

**时间**: 2026-05-04

**问题**: 播放器浏览时请求 `/Items/{id}/Images/Primary`，当图片不存在或远端返回 HTTP 404 时，项目仅做一次 TMDB 实时 API 代理。如果该代理也失败，则直接返回 `ErrorCode: NotFound`，不尝试修复 DB。下次请求仍 404。

### 修复内容

新增 `spawn_image_fallback_refresh()` 函数，在 `serve_item_image()` 的所有 404 出口异步触发完整 TMDB 刷新：
- 使用 `refresh_queue::try_begin_refresh()` 去重
- 后台调用 `do_refresh_item_metadata()` → TMDB API → 图片下载 → DB 路径更新
- Series 级联下载 Season/Episode 图片

| 文件 | 修改内容 |
|---|---|
| `backend/src/routes/images.rs` | 新增 `spawn_image_fallback_refresh()`；4 个 404 出口点插入异步触发 |

`cargo build` 通过（0 errors）。

---

## 第三十二轮修复：CollectionFolder（媒体库列表）响应精简，匹配官方 Emby 格式

**时间**: 2026-05-04

**问题**: 对比官方 Emby 响应与项目响应（`GET /emby/Users/{UserId}/Items` 顶层媒体库列表），发现多项差异：

| 差异 | 官方 Emby | 项目（修复前） |
|---|---|---|
| `ChildCount` | `1`（物理路径数） | `82781`（RecursiveItemCount 值，即递归条目总数） |
| 响应字段数 | ~12 个核心字段 | ~50+ 个字段（全量 BaseItemDto） |
| `StartIndex` | 不包含 | `0` |
| `UserData.PlayCount` | 不包含 | `0` |
| `UserData.ServerId` | 不包含 | 有 |
| `Taglines`/`People`/`Studios`/`MediaSources`/`Chapters` 等空数组 | 不包含 | `[]` |
| `CanDelete`/`SupportsSync`/`LockData` 等 | 不包含 | 有值 |

### 修复内容

#### 1. `ChildCount` 含义修正
- `library_to_item_dto_inner()`: `child_count` 改为 `locations.len().max(1)`（物理路径数），而非递归条目数
- `RecursiveItemCount` 字段在库列表中设为 None 以精简响应

#### 2. BaseItemDto 字段条件序列化
在 `models.rs` 中为以下字段添加 `skip_serializing_if`，使空值/None 时自动省略：

| 字段 | 变更 |
|---|---|
| `can_delete` | `bool` → `Option<bool>` + skip_serializing_if |
| `supports_sync` | `bool` → `Option<bool>` + skip_serializing_if |
| `taglines` | 添加 `skip_serializing_if = "Vec::is_empty"` |
| `remote_trailers` | 同上 |
| `people` | 同上 |
| `studios` | 同上 |
| `tag_items` | 同上 |
...
| `chapters` | 同上 |
| `locked_fields` | 同上 |
| `local_trailer_count` | `i32` → `Option<i32>` + skip_serializing_if |
| `part_count` | `i32` → `Option<i32>` + skip_serializing_if |
| `lock_data` | `bool` → `Option<bool>` + skip_serializing_if |
| `play_count` (UserItemDataDto) | `i32` → `Option<i32>` + skip_serializing_if |

#### 3. 库 DTO 精简
`library_to_item_dto_inner()` 重写，仅设置官方响应中存在的字段：
- `Name`, `ServerId`, `Id`, `Guid`, `CanDownload`, `IsFolder`, `Type`, `UserData`, `ChildCount`, `CollectionType`, `ImageTags`, `BackdropImageTags`
- 其余字段均设为 None/空值，由 skip_serializing_if 自动省略

#### 4. `StartIndex` 移除
`libraries_as_query_result_for_user()` 的 `start_index` 从 `Some(0)` 改为 `None`

#### 5. UserData 精简
库列表的 `UserData` 使用 `empty_user_data()`（PlayCount=None, ServerId=None），匹配官方：
```json
{"PlaybackPositionTicks": 0, "IsFavorite": false, "Played": false}
```

### 修复后项目 CollectionFolder 响应示例
```json
{
  "Name": "儿童",
  "ServerId": "C0EC7C09-F620-5784-96B9-2AFC127C5A2C",
  "Id": "497F6995-26D8-4730-ACF1-AE6F092BAD94",
  "Guid": "497F6995-26D8-4730-ACF1-AE6F092BAD94",
  "CanDownload": false,
  "IsFolder": true,
...
| 文件 | 修改内容 |
|---|---|
| `backend/src/models.rs` | BaseItemDto: 6 个字段改为 Option；9 个 Vec 字段添加 skip_serializing_if；UserItemDataDto.play_count 改 Option |
| `backend/src/repository.rs` | library_to_item_dto_inner 精简；所有 DTO 构建器适配新类型 |
| `backend/src/routes/items.rs` | libraries_as_query_result_for_user 移除 StartIndex；适配新类型 |
| `backend/src/routes/genres.rs` | 适配 can_delete/play_count 新类型 |
| `backend/src/routes/persons.rs` | 适配 can_delete 新类型 |
| `backend/src/routes/shows.rs` | 适配 local_trailer_count 新类型 |

`cargo check` 通过（0 errors）。

---

## 2026-05-04 Series/Season/Episode/PlaybackInfo 完整播放流程对比修复

### 对比范围

对比官方 Emby API 在以下 7 个端点的请求/响应：
1. `GET /emby/Users/{UserId}/Items?IncludeItemTypes=Series` — 剧集列表
2. `GET /emby/Users/{UserId}/Items/{SeriesId}?Fields=ShareLevel` — 剧集详情
3. `GET /emby/Shows/{SeriesId}/Seasons` — 季列表
4. `GET /emby/Shows/NextUp` — 下一集
5. `GET /emby/Shows/{SeriesId}/Episodes?Fields=MediaSources,...` — 分集列表
6. `POST /emby/Items/{ItemId}/PlaybackInfo` — 播放信息
7. `GET /emby/videos/{ItemId}/original.{ext}` — 视频流

### 发现的差异与修复

#### 1. Series 详情缺少 `DisplayOrder` 字段
- **官方**: `"DisplayOrder": "Aired"` — 用于告知播放器剧集排序方式
- **项目**: 数据库 `display_order` 列已存在，但 `DbMediaItem` 和 `BaseItemDto` 均未绑定此字段

**修复**:
- `DbMediaItem` + `MediaItemRow` 添加 `display_order: Option<String>`
- `BaseItemDto` 添加 `display_order: Option<String>` (skip_serializing_if = "Option::is_none")
- 所有 SELECT 查询（共 13 处）追加 `display_order` 列
- 所有 DTO 构建器（list / detail / library / root / missing_episode）赋值 `display_order`
- `From<MediaItemRow> for DbMediaItem` 同步新字段

#### 2. 分集列表 Fields=MediaSources 时 MediaSources 为空
- **官方**: 当 Fields 包含 `MediaSources` 时，Episode 列表每条都包含完整的 `MediaSources`（含 `MediaStreams`）
- **项目**: `media_item_to_dto_for_list` 始终返回 `media_sources: Vec::new()`，忽略 Fields 参数

**修复**:
- `repository.rs`: `media_sources_for_item` 改为 `pub` 可见性
- `routes/shows.rs`: 新增 `populate_media_sources_batch` 异步函数 + `fields_contain_media_sources` 辅助
- `get_episodes` 和 `get_episodes_by_season` 在构建 DTO 后，若 Fields 含 MediaSources，则批量填充每个 Episode 的 MediaSources

#### 3. PlaybackInfo DirectStreamUrl 格式不匹配
- **官方**: `/videos/{id}/original.{ext}?DeviceId=...&MediaSourceId=...&PlaySessionId=...&api_key=...`
- **项目**: `/Videos/{id}/original.{ext}?Static=true&DeviceId=...&MediaSourceId=...&PlaySessionId=...&api_key=...`

**修复**:
- `build_direct_stream_url`: 移除 `Static=true`，路径改为小写 `/videos/`
- 对应单元测试断言同步更新

#### 4. 其他已确认兼容的端点
| 端点 | 状态 | 说明 |
|---|---|---|
| Series 列表 | ✅ 兼容 | 项目返回字段是官方的超集，额外字段有 skip_serializing_if 控制 |
| Seasons | ✅ 兼容 | ParentBackdropItemId/SeriesPrimaryImageTag 等已正确填充 |
| NextUp | ✅ 兼容 | 路由 + 响应格式与官方一致 |
| PlaybackInfo | ✅ 修复后兼容 | DirectStreamUrl 格式已对齐 |
| 视频流路由 | ✅ 兼容 | `/videos/{id}/original.{ext}` 大小写变体均已注册 |

### 修改文件汇总

| 文件 | 修改内容 |
|---|---|
| `backend/src/models.rs` | `DbMediaItem` / `MediaItemRow` / `BaseItemDto` 添加 `display_order`；`From` impl 同步 |
| `backend/src/repository.rs` | 13 处 SELECT 添加 `display_order`；6 处 DTO 构建器赋值；`media_sources_for_item` 改 pub |
| `backend/src/routes/shows.rs` | 新增 `populate_media_sources_batch` / `fields_contain_media_sources`；两个 Episode 端点填充 MediaSources |
| `backend/src/routes/items.rs` | `build_direct_stream_url` 移除 `Static=true`、路径小写；测试同步 |

`cargo check` 通过（0 errors）。

---

## ⑮ 图片回退链路断裂：远端失败时未落回 TMDB / 占位图

**发现时间**: 2026-05-04  
**日志来源**: `movie-rust-20260504210928.log`

### 问题描述

分析日志发现 3 个断裂点导致图片 404/500 直接返回客户端，TMDB 回退和占位图机制被跳过：

#### 断点① — 远端连接失败(Internal)不走回退
- `serve_image_with_source` 对远端 Emby 的 HTTP 请求连接超时/断开时返回 `AppError::Internal`
- 但 L1469 仅捕获 `AppError::NotFound(_)`，`Internal` 直接作为 500 返回
- **结果**: 客户端看到 500 错误，无占位图

#### 断点② — 后台落盘 404 不清除 DB 死链
- `spawn_remote_image_persist` 后台任务：远端返回 404 时仅 WARN 日志
- DB 中 `image_primary_path` 仍保留死 URL
- 下次请求仍从 DB 读到死 URL → 再次 404 → 循环往复
- **结果**: 同一 item 2 分钟内被重试 5 次，浪费远端请求

#### 断点③ — 人物图片无 placeholder 回退
- `serve_person_image` / `get_item_image` 中人物分支：远端 403/404 直接返回错误
- 无 TMDB 回退，无占位图
- **结果**: 客户端显示破碎图标（44 次/2 分钟）

### 修复内容

#### 1. 主图片 handler 回退逻辑 (`images.rs`)
```rust
// 修复前：仅捕获 NotFound
if let Err(AppError::NotFound(_)) = &result {

// 修复后：NotFound 和 Internal 都走回退
let should_fallback = matches!(&result, Err(AppError::NotFound(_)) | Err(AppError::Internal(_)));
if should_fallback {
```

#### 2. 后台落盘失败清除死链 (`images.rs`)
```rust
Err(e) => {
    let is_not_found = matches!(&e, AppError::NotFound(_));
    if is_not_found {
        // 清除 DB 中死 URL → 下次请求 path_opt=None → 直接走 TMDB/占位图
        repository::update_media_item_image_path(&state.pool, item.id, &image_type, None, backdrop_index).await;
    }
}
```

#### 3. 人物图片占位图回退 (`images.rs`)
- `serve_person_image`: 解析失败 / 远端不可用 → `placeholder_image_response`
- `get_item_image` 人物分支: 同样回退到占位图
- `resolve_person_image_path`: 远端 403/404 时清除死链、返回 `Ok(None)` 而非 `Err`

### 修复后行为

| 场景 | 修复前 | 修复后 |
|---|---|---|
| 远端连接失败 | 500 Internal | TMDB 回退 → 占位图 |
| 远端 404 (首次) | TMDB 回退 ✓ 但下次仍重试 | TMDB 回退 ✓ + 清除死链 |
| 远端 404 (后续) | 循环重试死链 | path=NULL → 直接 TMDB/占位图 |
| 人物图片 403/404 | 404 破碎图标 | 占位图 + 清除死链 |

### 影响统计（2 分钟日志窗口）

- 远端图片重复 404: 10 次 → 修复后仅首次访问 1 次
- 人物图片 404: 44 次 → 修复后返回占位图
- 远端连接失败 500: 2 次 → 修复后返回占位图
